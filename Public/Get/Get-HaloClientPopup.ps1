...
function Get-HaloClientPopup {
    ...
    try {
        ...
        # If we're not in single-client mode, we want to return the full objects regardless to make sure we get all the details for popups
        if ($FullObjects -or !$ClientID) { 
            $AllClientResults = $ClientResults | ForEach-Object {             
                Get-HaloClient -ClientID $_.id
            }
            $ClientResults = $AllClientResults
        }
        Return $ClientResults | Select-Object -Expand popup_notes
    } catch {
